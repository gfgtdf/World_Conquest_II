...
local dialog = wesnoth.dofile("./help_dialog.lua")
...
function wesnoth.wml_actions.wc2_show_wocopedia(cfg)
	...
	local preshow = function()
		...
		if show_help_artifacts then
			-- TODO: handle 'not_available=player/enemy'
			local str_now_for_enemies = _ " (not available for enemies)"
			local str_now_for_players = _ " (not available for players)"
			...
			for i = 0, (wml.variables["wct.artifact.length"] - 1) do
				...
				if not_available.enemy then
					artifact_name = artifact_name .. str_now_for_enemies
				end
