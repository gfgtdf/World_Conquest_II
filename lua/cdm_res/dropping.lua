...
dropping.remove_all_items = function(filter)
	for k,v in pairs(cdm_dropping.field_data) do
		local start = #v
...
wesnoth.persistent_tags.next_item_name.write = function(add)
	add {
		field_data = cdm_helper.serialize(dropping.field_data),
		next_id = dropping.next_id,
	}
end
-- read might not be called if there is no [dropped_items] tag found
wesnoth.persistent_tags.next_item_name.read = function(cfg)
	dropping.field_data = cdm_helper.deseralize(cfg.field_data)
	dropping.next_id = cfg.next_id or 0
	dropping.remove_empty_lists()
end

cdm_on_event("moveto", function(event_context)
	...
	while i <= #entries do
		...
		wesnoth.fire_event("cdm_drop_pickup", x, y)
