...
function wesnoth.effects.cdm_optional_special(u, cfg)
	local attacks_to_add = {}
	for i = 1, #u.attacks do
		local attack = u.attacks[i]
		if cfg.range and cfg.range ~= attack.range then
			--Do nothing
		elseif cfg.type and cfg.type ~= attack.type then 
			--Do nothing
		else
			table.insert(attacks_to_add, attack.__cfg)
			for k,v in ipairs(cfg) do
				table.insert(attacks_to_add[#attacks_to_add], v)
			end
		end
	end
	for k,v in ipairs(attacks_to_add) do
		v.apply_to = "new_attack"
		wesnoth.add_modification(u, "object", { T.effect ( v)}, false)
	end
end

function wesnoth.effects.cdm_moves_defense(u, cfg)
...
function wesnoth.effects.cdm_min_resistance(u, cfg)
...
function wesnoth.effects.cdm_min_defense(u, cfg)
	local defense_new = {}
...
function wesnoth.effects.cdm_update_aura(u, cfg)
...
function wesnoth.effects.cdm_overlay(u, cfg)
	if cfg.add then
